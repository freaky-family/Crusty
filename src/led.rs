...
pub async fn led_task(mut servos: Servos, ledc: Ledc<'static>) {
    ...
    loop {
        set_position(&mut cl, &mut cr, &mut duty_info, &delay, &mut is_high, &mut old_state);
        Timer::after(Duration::from_millis(50)).await;
    }
}

fn set_position(cl: & mut [channel::Channel<'_, HighSpeed>; 4],
    cr: & mut [channel::Channel<'_, HighSpeed>; 4],
    duty_info: & mut DutyInfo, delay: &Delay, is_high: & mut u8,
    old_state: & mut u8) {
